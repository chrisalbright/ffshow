...
public class MovieController {
  ...
  public Flux<ShowTime> getMovieShowTimes(@PathVariable Integer id) {
    return Flux.from(movieService.getMovieById(id)).flatMap(movie -> Flux.fromIterable(movie.getShowTimes()));
  }
  ...
  public Mono<ShowTime> addNewShowtime(@PathVariable Integer id, @RequestBody ShowTime showTime) {
    return movieService.getMovieById(id).map(showTime::withMovie).flatMap(showTimeService::addShowTime);
  }
